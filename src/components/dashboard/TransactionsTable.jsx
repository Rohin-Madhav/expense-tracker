...
import { Trash2, ArrowUpRight, ArrowDownLeft, ChevronLeft, ChevronRight } from 'lucide-react';
import { format, parseISO } from 'date-fns';

const PAGE_SIZE = 8;

export default function TransactionsTable({ transactions: propTransactions, showPagination = true }) {
    const { transactions: allTransactions, deleteTransaction, formatCurrency } = useApp();
    const data = propTransactions ?? allTransactions;
    const [page, setPage] = useState(1);

    const totalPages = Math.ceil(data.length / PAGE_SIZE);
    const paginated = showPagination ? data.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE) : data.slice(0, PAGE_SIZE);
                ...
                <>
                    ...
                    <div className="md:hidden space-y-3">
                        {paginated.map((txn) => {
                            const cat = getCategoryById(txn.category);
                            return (
                                <div key={txn.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-700/30">
                                    <div
                                    ...
                                    <button onClick={() => deleteTransaction(txn.id)} className="btn-danger p-1.5">
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                            );
